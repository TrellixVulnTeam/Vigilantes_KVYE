...
def plurality_vote(images, priority=None): # Please use an odd number of
    ...
    for image in images:
        ...
        p = 1 if not priority else priority[i]
        for k in range(p):
    ...
    for file in glob.glob("plate_test/*"):
        ...
        for x in sizes:
            for y in sizes:
                ...
                #choices.append(cv2.resize(aggressive_cut, None, fx=x, fy=y))
                priority.append(1)
